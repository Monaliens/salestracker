  ...
  if (interaction.guildId && !isGuildWhitelisted(interaction.guildId)) {
    if (interaction.isCommand()) {
      await interaction.reply({ 
        content: '⛔ This server is not whitelisted. Bot functionality is restricted. Please contact the bot owner to request access.', 
        ephemeral: true 
      });
    }
    return;
  ...
  try {
    ...
    switch (commandName) {
        ...
        try {
          ...
          const trackingEmbed = new EmbedBuilder()
            ...
            .addFields(
              ...
              { name: 'Notification Channel', value: `<#${channelOption.id}>`, inline: true },
      ...
      case 'checksales': {
        ...
        await interaction.deferReply({ ephemeral: true });
        try {
          await interaction.editReply({ content: '🔍 Checking for new sales for this server... Please wait...', ephemeral: true });
          
          // Only check sales for the current server, not all servers
          ...
          if (!serverConfig || !serverConfig.trackedCollections || Object.keys(serverConfig.trackedCollections).length === 0) {
            await interaction.editReply({ 
              content: '❌ No collections are being tracked on this server or notification channel is not set.', 
              ephemeral: true 
            });
            return;
          }
          ...
          try {
            // Only check sales for this specific server
            await checkAndNotifySales(serverId);
            await interaction.editReply({ 
              content: `✅ Sales check complete! Check your notification channel for any new sale notifications.`, 
              ephemeral: true 
            });
          } catch (checkError) {
            console.error(`Error checking sales for server ${serverId}:`, checkError);
            await interaction.editReply({
              content: '❌ Error checking for sales. Check the console logs for details.',
              ephemeral: true
            });
          }
        } catch (error) {
      ...
      case 'help':
        const helpEmbed = new EmbedBuilder()
          ...
          .addFields(
            { name: '/track <channel> <collection>', value: 'Track sales for a specific collection and set notification channel (Admin only)' },
...
async function checkAndNotifySales(serverId) {
  try {
    ...
    const serverConfig = serverConfigs[serverId];
    if (!serverConfig || !serverConfig.trackedCollections || 
        Object.keys(serverConfig.trackedCollections).length === 0 || 
        !serverConfig.notificationChannelId) {
      console.log(`⏩ Skipping guild ${serverId}: no collections tracked or no notification channel set`);
      return;
    }
    ...
    console.log(`Processing sales for guild: ${guild.name} (${serverId})`);
    const collectionAddresses = Object.keys(serverConfig.trackedCollections);
    console.log(`Total tracked collections: ${collectionAddresses.length}`);

    // Process each collection for this server
    for (const collectionAddr of collectionAddresses) {
      console.log(`Checking sales for collection: ${collectionAddr}`);
      const sales = await fetchReservoirSales([collectionAddr]);
      
      if (sales.length === 0) {
        console.log(`No sales found for collection ${collectionAddr}`);
        continue;
      }
      ...
      for (const sale of sales) {
        ...
        // Check if this sale has already been notified about for this collection
        // This check is now based on the collection + sale ID rather than just sale ID
        if (lastProcessedSales.processedSales.has(`${collectionAddr}_${saleId}`)) {
          console.log(`Sale ${saleId} already processed for collection ${collectionAddr}, skipping`);
          continue;
        }
        
        console.log(`New sale found for collection ${collectionAddr}: ${saleId}`);
        ...
        try {
          ...
          console.log(`📤 Sending notification to #${channel.name} in ${guild.name}`);
          await channel.send({ embeds: [embed] });
          console.log(`✅ NOTIFICATION SENT SUCCESSFULLY for collection ${collectionAddr}!`);
          
          // Mark as notified with collection-specific ID
          lastProcessedSales.processedSales.set(`${collectionAddr}_${saleId}`, Date.now());
...
// Add validateWhitelist function
function validateWhitelist() {
  console.log(`\n======= WHITELIST CONFIGURATION =======`);
  
  if (!process.env.WHITELISTED_GUILDS || process.env.WHITELISTED_GUILDS.trim() === '') {
    console.log(`⚠️ WHITELIST NOT CONFIGURED: Bot will work in ALL servers it joins`);
    console.log(`To enable whitelist, add guild IDs to WHITELISTED_GUILDS in .env file`);
  } else {
    console.log(`✅ WHITELIST ENABLED: ${config.whitelistedGuilds.length} servers authorized`);
  ...
  if (client.guilds.cache.size > 0) {
    ...
    if (nonWhitelisted.size > 0) {
      console.log(`\n⚠️ BOT IS IN ${nonWhitelisted.size} NON-WHITELISTED SERVERS:`);
